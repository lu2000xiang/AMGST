...
if __name__ == "__main__":
    # -------------------------- set running env1   ironment ------------------------- #

    parser = argparse.ArgumentParser()
    parser.add_argument("-d", "--dataset", type=str, default="PEMS08")
    parser.add_argument("-g", "--gpu_num", type=int, default=0)
    parser.add_argument("-sc", "--sc_filename", type=str, default="data/PEMS08/SCORR_PEMS08_0.6_C16.npy")
    ...
    sc_mx = np.load(args.sc_filename)
    # support = multichannel_norm_adj(sc_mx).astype(np.float32)
    support = sc_mx[0]
    support = [asym_adj(support), asym_adj(np.transpose(support))]
    support = [torch.tensor(i).to(DEVICE) for i in support]


    dataset = args.dataset
    dataset = dataset.upper()
    data_path = f"../data/{dataset}"
    model_name = AMGST.__name__
    ...
    model = AMGST(**cfg["model_args"], support=support)
    # ------------------------------- make log file ------------------------------ #
